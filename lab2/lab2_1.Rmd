...
Code chunk (on mac... command + option + i) 
run (on mac... command + return)
```{r}
...
```{r}
#mean(2+8+2) outputs a 12.... 
mean(c(2, 8,2)) #c stands for combine and creates a vector
```

Why does this not work?
```{r}
mean(2, 8, 2) #doesn't work because not in a vector
```
...
```{r}
mean(c(venom_GT, chiron, veyron)) #need to use vector 
```
...
```{r}
my_integer <- as.integer(my_numeric) #important when we need to change class 
```
...
```{r}
anyNA(my_missing) #any NA in object or vector
```
...
This interpret tells us that our mean is not able to be calculated because of the missing data in the vector. 
NAs ends up heavily influencing the behavior of functions. 

4. Recalculate the mean using the following code chunk. Why is the useful?  
```{r}
mean(new_vector, na.rm = TRUE) #removes NA values... TRUE and T are the same 
```
It is useful because it lets you bypass the NA in the calculations. 