...
penguins %>% 
  ...
  arrange(body_mass_g) #defaults to ascending 
```
...
# can also do filter(island==c("Biscoe", "Dream"))... same result 
#can alsooo do filter(island %in% c("Bisceo", "Dream"))
```
...
```{r}
penguins %>% 
  distinct(species, island, .keep_all = T) #.keep_all makes sure other parts of row are outputed too 
```
...
```{r}
mammals <-read.csv("data/mammal_lifehistories_v2.csv") %>% clean_names()
glimpse (mammals)
```
...
```{r}
mammals %>% 
  mutate(across(everything(), tolower)) #changes numeric to chr... problems... be careful with numeric variables
```
...
mammals <- mammals %>% #need to put it in for it to be saved properly 
...
```{r}
mammals %>% 
  select(gestation, mass) %>% 
  ggplot(data = mammals, mapping=aes(x = gestation, y = mass)) + 
  geom_point() + 
  geom_smooth(method="lm", se=F) + 
  scale_y_log10() + 
  scale_x_log10() +
  labs(title="Gestation vs. newborn mass", x="Gestation", y="Newborn mass")
```
...
glimpse(mammals)
mammals %>%  
  select(order, species, max_life) %>% 
  mutate(max_life_new=max_life/12) %>% #month to year
  slice_max(max_life, n = 1)
```
The Cetacea physalus has the longest life span. 

5. Do larger mammals live longer?
```{r}
mammals %>% 
  select(gestation, mass) %>% 
  ggplot(data = mammals, mapping=aes(x = mass, y = max_life)) + 
  geom_point() + 
  geom_smooth(method="lm", se=F) + 
  scale_x_log10() + 
  scale_y_log10() +
  labs(title="Mass vs. life span", x="Mass", y="Life span")
```
