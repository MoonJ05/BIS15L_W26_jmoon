...
```{r}
filter(penguins, body_mass_g %in% c(5250, 5500)) #restricting to either of these two values 
```
...
```{r}
filter(penguins, body_mass_g %in% c(5250, 5400, 5350, 5250, 5300, 5450, 5500))
```

You can also extract observations "near" a certain value but you need to specify a tolerance.  
```{r}
filter(penguins, near(body_mass_g, 5400, tol=100)) #splits into 50 on left, 50 on right... splits difference on both sides! so 5350 to 5450 
```
...
```{r}
getwd()
fish <- read.csv("data/Gaeta_etal_CLC_data.csv") #don't need absolute, only from working directory 
```
...
```{r}
names(fish)
filter(fish, near(scalelength, 8, 0.5)) #0.25 on each side 
```
...
```{r}
filter(penguins, species =="Chinstrap" & body_mass_g > 4500) #& is used to make sure both conditions are met 
```
...
```{r}
names(penguins)
filter(penguins, body_mass_g > 5000, (bill_length_mm>50 | bill_depth_mm > 18)) # in this case , and & act the same 
```
...
```{r}
penguins %>% #shift command m outputs %>%  .. general rule select goes first
...
penguins %>% 
  select(species, body_mass_g) %>% 
  filter(species == "Gentoo") %>% 
  arrange(body_mass_g) #defaults to ascending order 
```
...
penguins %>% 
  select(species, body_mass_g) %>% 
  filter(species == "Gentoo") %>% 
  slice_max(body_mass_g, n=1) #and slice_min
  #slice_head() #there is also slice_tail() ... 
```
...
```{r}
fish %>% 
  select(fish_id, radii_length_mm, scalelength) %>% #don't need to specify data, since already in it 
  filter(fish_id == 300) %>% 
  arrange(radii_length_mm) #in ascending order 
```
...
```{r}
fish %>% 
  filter((lakeid == "AL" | lakeid == "AR") & between(radii_length_mm, 2, 4)) %>% #can break it up for clarity instead of efficiency 
