...
penguins <- penguins #imports data so you can view it in R 
```
...
```{r}
relocate(penguins, year) #moves column to the front 
```
...
```{r}
order_genus_bodywt_msleep <- select(msleep, order, genus, bodywt)
order_genus_bodywt_msleep
```
