...
```{r message=FALSE, warning=FALSE}
life_history <- read_csv("data/mammal_lifehistories_v2.csv", na="-999") %>% # removes the weird -999 to na
  clean_names()
```
...
life_history %>% 
  ...
  summarize(avg_mass = mean(mass,  na.rm = T)) %>% 
  arrange(desc(avg_mass)) %>% 
  ggplot(aes(x=reorder(order, avg_mass), y = avg_mass)) + #reorders! 
  geom_col(aes(fill = order)) + 
  coord_flip()+
  scale_y_log10()
```
...
```{r}
ggplot (data = life_history, 
        mapping=aes(x=gestation, y=wean_mass)) + 
  geom_point (na.rm = T) + 
  geom_smooth (method = lm, se = T) + 
  scale_y_log10() + 
  scale_x_log10() +
  labs (title = "Gestation time vs. wean mass", x = "Gestation time", y = "Wean Mass")
```
...
```{r}
ggplot (data= life_history, 
	mapping=aes(x= order, y=log10(mass))) + #another way of scaling! 
	geom_boxplot(mapping=aes(fill = order)) +
	labs (title = "mass vs. order", x= "order", y = "mass") + 
  coord_flip()
```
...
```{r}
#scatter plot since two continuous 
life_history %>% 
