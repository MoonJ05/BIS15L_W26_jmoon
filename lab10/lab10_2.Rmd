...
```{r}
p+geom_point(size=3) #can over plot... but storing = easier to play with later 
#geom_jitter aside from geom_point = random noise that can help with overplotting
```
...
```{r}
p + 
  geom_point (size = 3, color = "black", alpha=0.8, shape = 24, fill = "blue") #alpha controls transparency
```
...
```{r}
homerange %>% 
  count(taxon, sort = T) #sorts! whoaaa 
```
...
homerange %>% 
  ggplot(aes(x = taxon, fill=trophic_guild)) + #abomination of the century 
  geom_bar(position = "dodge")+ #always better to hae side by sie 
  coord_flip()+ 
  labs(title = "Observations by Taxon", 
       x = "Taxonomic group",
       y=NULL) # no label at all whoa
