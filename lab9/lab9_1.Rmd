...
3. Use the `pivot_longer()` function of `tidyr` to transform data from wide to lonfg format.  
...
Not possible.. we need to make data long so that the drug types are their own seperate variable too. This would let us compare the columns. 
Otherwise we can't really seperate the columns 
...
#makes the data longer! 
```

```{r}
#now trying to make the plot... 
heartrate_long %>% 
...
```{r}
relig_income %>% 
  pivot_longer(-religion, #doesn't change
             names_to="income", #give new column name
             values_to="total") #move the counts into new column 
```
...
```{r}
billboard %>% 
  pivot_longer(wk1:wk76, #specifying specific range we want to make longer 
               names_to="week",
               values_to = "rank", 
               values_drop_na = T)
```
...
days are actually a variable name, so it should be in a column. 
...
qpcr_tidy <- qpcr_untidy %>% 
  pivot_longer(
    ...
    names_sep = "_", #tells us what the names are seperated by!
