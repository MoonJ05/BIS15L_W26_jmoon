...
No, need to seperate into years (aka period)

```{r}
expenditures_cleaned <- expenditures %>% 
  pivot_longer(-expenditure,
               names_to="period",
               values_to = "dollars_billions", 
               values_drop_na = T)
...
expenditures_cleaned %>% 
  ...
  arrange(dollars_billions)  #sorts in ascending order
```
...
```{r}
heartrate2 %>% 
  separate(patient, into=c("patient", "gender"), sep=-1) #using last letter! 
```
...
heartrate2 %>% 
  ...
  mutate(patient=str_remove(patient, "_")) #mutates column,by removing _ in patients
```
...
beachbugs_wide #when you knit it... get rid of it so that it doesn't kill your amazing papers-- 
```
