...
```{r}
leaflet(spiders) %>% 
  addTiles() %>%  # This uses the default OpenStreetMap tiles
  addCircleMarkers(
    lng = ~longitude, 
    lat = ~latitude,
    radius = 2,
...
grizzly %>% 
  summary()
lonbear <- c(-166.2, -131.3)
latbear <- c(55.02,70.37)
bboxbear <- make_bbox(lonbear, latbear, f=.03) 
```

3. Load a map from `stadiamaps` in a `stamen_terrain` projection and display the map.
```{r}
map_bear <- get_stadiamap(bboxbear, maptype = "stamen_terrain", zoom=5)
```
...
```{r}
ggmap(map_bear)+
  geom_point(data=grizzly, aes(longitude, latitude), size=.5, color="black", alpha=.8)+
  labs(x="Longitude",y="Latitude",title="Bare")
```
