...
```{r}
ymd_hms(dateformat5) #usually use as mutate on a column
```
...
```{r, message = FALSE}
glimpse(spider_locations)
glimpse(spider_records)
```

2. Join the files using an appropriate key and join type.
```{r}
spider <- full_join(spider_locations, spider_records, by = "accession") #left join could work too
names(spider)#check
```

```{r}
anti_join(spider_locations, spider_records, by = "accession")
#check for mismatch (troubleshooting)
```
