...
```{r}
#plot type... honestly comparative box plot probably works best (species (categorical) and body mass (discrete))
ggplot(data=penguins, #specify the data
       mapping=aes(x=species, y=body_mass_g)) #map the aesthetics 
#need to be spelled exactly the same
...
```{r}
ggplot(data=penguins, mapping=aes(x=species, y=flipper_length_mm)) + #specify data and map aesthetics
  geom_boxplot() #generate box plot
#again using box plot since categorical and discrete 
```
...
```{r}
#continuous and continuous so scatter plot 
names(penguins)
```
...
ggplot(data=penguins, 
  ...
  geom_point() #scatter plot comes in many forms... this is most common 
```
...
ggplot(data=penguins, 
       mapping=aes(x=body_mass_g, y=flipper_length_mm))+ 
  geom_point(na.rm=T) #scatter plot comes in many forms... this is most common 
```
...
ggplot(data=penguins, 
       mapping=aes(x=body_mass_g, y=flipper_length_mm))+ 
  geom_point(na.rm=T)+ #scatter plot comes in many forms... this is most common
  geom_smooth(method=lm, se=T) #just another layer (linear model (lm), standard error (se)) 
```
...
```{r}
ggplot(data=penguins, 
       mapping=aes(x=body_mass_g, y=flipper_length_mm, color=species))+ #add another variale for color
  geom_point(na.rm=T)+ 
...
ggplot(data=penguins, 
       mapping=aes(x=body_mass_g, y=flipper_length_mm))+ 
  geom_point(mapping=aes(color=species))+ #added color to this layer  
  geom_smooth(method=lm, se=T)
```
...
ggplot(data=penguins, 
       mapping=aes(x=body_mass_g, y=flipper_length_mm))+ 
  geom_point(mapping=aes(color=species, shape=species))+ #added shapes too! know your audience. usualy categorical 
  geom_smooth(method=lm, se=T)
```
...
```{r}
#bar plot is a count of categorical 
ggplot(data=penguins, 
       mapping=aes(x=species))+ #since count, no need to tell what y is! 
  geom_bar()+
  labs(title="# of individuals per species")
```
...
ggplot(data=penguins, 
  ...
  geom_bar(mapping=aes(color=species))+ #works but not ideal
  labs(title = "#Penguins for each species")
```
...
ggplot(data=penguins, 
       mapping=aes(x=species))+ 
  geom_bar(mapping=aes(fill=species))+ #command to color the whole bar
  labs(title = "#Penguins for each species")
```
...
ggplot(data=penguins, 
  ...
  labs(title = "#Penguins on each island")
```
...
ggplot(data=penguins, 
       mapping=aes(x=island))+ 
  geom_bar(mapping=aes(fill=species), position="dodge")+ #add position to make it side by side 
  labs(title = "#Penguins for each species by island")
```
## Wrap-up  
